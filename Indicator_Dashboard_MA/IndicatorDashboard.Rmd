...
resource_files:
  ...
  - "Data/rnatearth_Mariana_coast.RData"
  - "Data/rnatearth_Mariana_land.RData"
...
land_proj <- readRDS(here('Data', 'rnatearth_Mariana_land.RData'))
coast_proj <- readRDS(here('Data', 'rnatearth_Mariana_coast.RData'))
...
output$season_desc <- renderUI(
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("The seasonal cycle plot shows ocean color over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual ocean color cycle (greatest in January, least in April).")     
  } else if (input$indicator == "Rainfall") {
    markdown("The seasonal cycle plot shows rainfall over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual cycle for rainfall (highest in August, lowest in March). ")    
  }
)
...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    p <- ggplot() + 
      ...
      geom_rect(aes(xmin = -37, xmax = -34, ymin = 13, ymax = 19), color = "black", fill = NA, linewidth = 0.5) + #AS box
      annotate("text", x = -36, y = 20, label = "Marianas Grid", size = 3.2, color = "black") +
...
output$map_desc <- renderUI( 
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("This map shows the climatological average SST (1985-2021).  The data in the graphs above is averaged over the Marianas archipelago.")
  } else if (input$indicator == "Chl") {
    markdown("This map shows the climatological average ocean color (1998-2021).  The data in the graphs above is averaged over the Marianas archipelago.")    
  }
)
