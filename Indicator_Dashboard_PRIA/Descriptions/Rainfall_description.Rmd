...
The rainfall over the Pacific Remote Island Area (PRIA) was `r signif(ann_mean_RptYr,3)` mm day^-^^1^ in `r RptYr`. Monthly rainfall ranged from `r signif(monthly_min_RptYr,3)`–`r signif(monthly_max_RptYr,3)` mm day^-^^1^ during the year, within the range of values observed over the period of record (`r signif(monthly_min_PrevYrs,3)`–`r signif(monthly_max_PrevYrs,3)`  mm day^-^^1^). Over the period of record, annual rainfall has shown weak but significant linear decrease at a rate of `r abs(signif(trend, 3))` mm day^-^^1^ yr^-^^1^, and rainfall anomalies have declined by `r abs(signif(delta_rain_anom_lm, 1))` mm day^-^^1^. Overall rainfall was near average in 2024.
...
Region: Pacific Remote Island Area (PRIA) Grid (1°S – 7°N, 177° – 159°W)
