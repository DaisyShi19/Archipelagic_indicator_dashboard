...
resource_files:
  ...
  - "Data/rnatearth_PRIA_coast.RData"
  - "Data/rnatearth_PRIA_land.RData"
...
land_proj <- readRDS(here('Data', 'rnatearth_PRIA_land.RData'))
coast_proj <- readRDS(here('Data', 'rnatearth_PRIA_coast.RData'))
...
output$season_desc <- renderUI(
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("The seasonal cycle plot shows SST over time by month.  The horizontal black lines are the time series average for each month.  This figure shows both the annual SST cycle (coolest in February, warmest in July) and that SST is increasing over time in all months of the year.")
  } else if (input$indicator == "Chl") {
    markdown("The seasonal cycle plot shows ocean color over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual ocean color cycle (greatest in June, least in October).")     
  } else if (input$indicator == "Rainfall") {
    markdown("The seasonal cycle plot shows rainfall over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual cycle for rainfall (highest in April and May, lowest in September). ")    
  }
)
...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    #make base map
    p <- ggplot() + 
      geom_rect(aes(xmin = 120, xmax = 260, ymin = -5, ymax =  55.2035), #rectangle for coloring ocean 
                color = NA, fill = oceans[1])  +
      geom_rect(aes(xmin = 3+180, xmax = 22+180, ymin = -1, ymax = 7), color = "black", fill = NA, linewidth = 0.5) + #AS box
      annotate("text", x = 5+180, y = 8.2, label = "PRIA Grid", size = 3.2, color = "black") +
    ...
    if (input$indicator == "CO2"){
      ...
      p <- p +       
        ...
        coord_sf(xlim = c(120, 260), ylim = c(-5, 55.2035), expand = F) +
...
output$map_desc <- renderUI( 
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("This map shows the climatological average SST (1985-2021).  The data in the graphs above is averaged over the Pacific Remote Island Area (PRIA).")
  } else if (input$indicator == "Chl") {
    markdown("This map shows the climatological average ocean color (1998-2021).  The data in the graphs above is averaged over the Pacific Remote Island Area (PRIA).")    
  }
)
