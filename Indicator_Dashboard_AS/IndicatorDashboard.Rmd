...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    p <- ggplot() + 
      ...
      geom_rect(aes(xmin = 180-172.5, xmax = 180-165, ymin = -17.6, ymax = -10.5), color = "black", fill = NA, linewidth = 0.5) + #AS box
      annotate("text", x = 180-173, y = -10, label = "American Samoa Grid", size = 3.2, color = "black") +
...
output$map_desc <- renderUI( 
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("This map shows the climatological average SST (1985-2021).  The data in the graphs above is averaged over the Main Hawaiian islands.")
  } else if (input$indicator == "Chl") {
    markdown("This map shows the climatological average ocean color (1998-2021).  The data in the graphs above is averaged over the Main Hawaiian islands.")    
  }
)
