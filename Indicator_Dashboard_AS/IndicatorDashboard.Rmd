...
output: 
  flexdashboard::flex_dashboard:
    source_code: "https://github.com/DaisyShi19/Archipelagic_indicator_dashboard"
    orientation: rows
...
resource_files:
  ...
  - "Data/rnatearth_tcs_coast.RData"
  - "Data/rnatearth_new_land.RData"
  - "Data/rnatearth_AS_coast.RData"
  - "Data/rnatearth_AS_land.RData"
  ...
  - "Images/Rainfall-Indicator-Table-2024-Monthly-Variables.png"
  ...
  - "Descriptions/Rainfall_description.md"
---
...
include_anom_list <- c("SST", "CO2", "Chl", "Rainfall") #indicators that have an anomaly plot
include_anom_map_list <- c("SST", "Chl") #indicators that have an anomaly map
...
land_proj <- readRDS(here('Data', 'rnatearth_AS_land.RData'))
coast_proj <- readRDS(here('Data', 'rnatearth_AS_coast.RData'))
land_proj_glb <- readRDS(here('Data', 'rnatearth_new_land.RData'))
#coast_proj_glb <- readRDS(here('Data', 'rnatearth_tcs_coast.RData'))
...
The dashboard data match the __`r report_year`__ archipelagic SAFE report.  Dashboard data will be updated each summer following publication of the SAFE report.
...
output$ts_desc <- renderUI( 
  ...
  } else if (input$indicator == "Rainall") {
    markdown("Monthly and annual rainfall")    
...
output$anom_desc <- renderUI(
  ...
  } else if (input$indicator == "Rainfall") {
    markdown("Monthly and annual rainfall with the seasonal cycle removed, as well as the long-term trend in this anomaly series")    
  }
)
...
output$season_desc <- renderUI(
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("The seasonal cycle plot shows SST over time by month.  The horizontal black lines are the time series average for each month.  This figure shows both the annual SST cycle (coolest in August, warmest in April) and that SST is increasing over time in all months of the year.")
  } else if (input$indicator == "Chl") {
    markdown("The seasonal cycle plot shows ocean color over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual ocean color cycle (greatest in June, least in February).")     
  } else if (input$indicator == "Rainfall") {
    markdown("The seasonal cycle plot shows rainfall over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual cycle for rainfall (highest in February, lowest in August). ")    
  }
)
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    if (input$indicator == "SST"){
      ...
      fill_scale <- scale_fill_gradientn(name = input$indicator, trans = log10_trans(),  
                                          colours = pal,  
                                  limits = c(log10(0.01), log10(1.5)),oob = squish) #log scale
    ...
    #make base map
    p <- ggplot() + 
      geom_rect(aes(xmin = 120, xmax = 260, ymin = -20.0745, ymax =  55.2035), #rectangle for coloring ocean #change ymin to fit AS domain -25
                color = NA, fill = oceans[1])  +
      geom_rect(aes(xmin = 7.5+180, xmax = 15+180, ymin = -17.6, ymax = -10.5), color = "black", fill = NA, linewidth = 0.5) + #AS box
      annotate("text", x = 7+180, y = -9.3, label = "American Samoa Grid", size = 3.2, color = "black") +
      geom_vline(xintercept = c(0+180, 60+180), color="black", linewidth = 0.1) + #manually add graticules (hidden by ocean rectangle)
      geom_hline(yintercept = seq(-20, 60, 10), color="black", linewidth = 0.1) +
      xlab("") + ylab("") +
      theme_bw()
    ...
    if (input$indicator == "CO2"){
      ...
      p <- p +       
        ...
        geom_rect(aes(xmin = ext$xmin, xmax =120, ymin = ext$ymin, ymax =  ext$ymax),  color = NA, fill = "white")  +
        geom_rect(aes(xmin = 260, xmax =ext$xmax+8,  ymin = ext$ymin, ymax =  ext$ymax),  color = NA, fill = "white")  +
        #geom_sf(data = coast_proj_glb, color = "black", linewidth = 0.5) +
        coord_sf(xlim = c(120, 260), ylim = c(-20.0745, 55.2035), expand = F) +
        annotate(geom = "point", x = 24.4+180, y = 19.5, color = crustacean[2]) +
        annotate(geom = "text", x = 33.5+1+180, y = 19.5-1, color = "black", size = 3.2,
                 label = "Mauna Loa\nObservatory")
      ...
      p <- p + 
        ...
        #geom_sf(data = coast_proj_glb, color = "black", linewidth = 0.5) +
        coord_sf(xlim = c(120, 260), ylim = c(-20.0745, 55.2035), expand = F) +
        annotate(geom = "point", x = 22.0+180, y = 22.75, color = crustacean[2]) +
        annotate(geom = "text", x = 16.0-3+180, y = 22.75-4, color = "black", size = 3.2,
                 label = "Station\nALOHA")
      ...
      x_mod = x-30
      g$x_mod$data[[9]]$text <- "158.0W, 22.75N<br />Station ALOHA"
      ...
      p <- p + 
        geom_rect(aes(xmin = 60+180, xmax = 10+180, ymin = -5, ymax = 5), 
        ...
        coord_sf(xlim = c(120, 260), ylim = c(-20.0745, 55.2035), expand = F) +
        annotate("text", x = 35+180, y = 7, label = "Niño 3.4 Region", size = 3.2, color = "black")
      ...
      p <- p + 
        geom_rect(aes(xmin = 120, xmax = 260, ymin = 20, ymax = 55.2035), 
                         color = crustacean[3], fill = crustacean[2], 
                         alpha = 0.5, linewidth = 0.5) +
        geom_sf(data = land_proj_glb, fill = "#A5A5A5", color = "#A5A5A5", linewidth = 0.5) +
        geom_rect(aes(xmin = ext$xmin, xmax =120, ymin = ext$ymin, ymax =  ext$ymax),  color = NA, fill = "white")  +
...
output$map_desc <- renderUI( 
  ...
  if (input$indicator == "CO2") {
    markdown("Atmospheric CO2 data are gathered at the Mauna Loa Observatory on the island of Hawaiʻi.  The Main Hawaiian islands are also shown for reference. ")
  } else if (input$indicator == "pH") {
    markdown("Oceanic pH data are gathered at Station ALOHA, north of the island of Oahu.  The Main Hawaiian islands are also shown for reference.")
  } else if (input$indicator == "ENSO") {
    markdown("The Oceanic Niño Index is measured across the Niño 3.4 Region.  The Main Hawaiian islands are also shown for reference.")
  } else if (input$indicator == "PDO") {
    markdown("The Pacific Decadal Index is measured across the North Pacific basin, north of 20°N.  The Main Hawaiian islands are also shown for reference.")
  } else if (input$indicator == "SST") {
    markdown("This map shows the climatological average SST (1985-2009).  The data in the graphs above is averaged over American Samoa.")
  } else if (input$indicator == "Chl") {
    markdown("This map shows the climatological average ocean color (1998-2009).  The data in the graphs above is averaged over American Samoa.")    
  }
)
...
output$anom_map_desc <- renderUI(
  ...
  #indicator-specific descriptions for seasonal plot
  if (input$indicator == "SST") {
    markdown("This anomaly map shows the difference between SST in the report year (2024) and the long-term average (1985-2009).  Areas that were warmer in 2023 are red and areas that were cooler are blue.")
  } else if (input$indicator == "Chl") {
