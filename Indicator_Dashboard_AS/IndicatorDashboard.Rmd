...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    #make base map
    p <- ggplot() + 
      geom_rect(aes(xmin = ext$xmin, xmax = ext$xmax, ymin = -20, ymax = ext$ymax), #rectangle for coloring ocean #change ymin to fit AS domain -25
                color = NA, fill = oceans[1])  +
      geom_rect(aes(xmin = 7.5, xmax = 15, ymin = -17.6, ymax = -10.5), color = "black", fill = NA, linewidth = 0.5) + #AS box
      annotate("text", x = 7, y = -9.5, label = "American Samoa Grid", size = 3.2, color = "black") +
    ...
    if (input$indicator == "CO2"){
      ...
      p <- p +       
        ...
        coord_sf(xlim = c(ext$xmin, ext$xmax), ylim = c(-20, ext$ymax), expand = F) +
        annotate(geom = "point", x = 24.4, y = 19.5, color = crustacean[2]) +
        annotate(geom = "text", x = 33.5+1, y = 19.5-1, color = "black", size = 3.2,
                 label = "Mauna Loa\nObservatory")
      ...
      p <- p + 
        ...
        annotate(geom = "text", x = 16.0-3, y = 22.75-4, color = "black", size = 3.2,
                 label = "Station\nALOHA")
      ...
      p <- p + 
        ...
        geom_sf(data = land_proj_glb, fill = "#A5A5A5", color = "#A5A5A5", linewidth = 0.5) +
        #geom_sf(data = coast_proj_glb, color = "black", linewidth = 0.5) +
        coord_sf(xlim = c(ext$xmin, ext$xmax), ylim = c(-22, ext$ymax), expand = F) +
