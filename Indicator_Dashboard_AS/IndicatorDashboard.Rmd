...
output$season_desc <- renderUI(
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("The seasonal cycle plot shows rainfall over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual cycle for rainfall (highest in December, lowest in June). ")    
  }
)
...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    #make base map
    p <- ggplot() + 
      geom_rect(aes(xmin = ext$xmin, xmax = ext$xmax, ymin = -5, ymax = ext$ymax), #rectangle for coloring ocean #change ymin to fit AS domain
                color = NA, fill = oceans[1])  +
      geom_rect(aes(xmin = 19, xmax = 26, ymin = -17.6, ymax = -10.5), color = "black", fill = NA, linewidth = 0.5) + #AS box
      annotate("text", x = 32.5, y = 24.5, label = "American Samoa Grid", size = 3.2, color = "black") +
