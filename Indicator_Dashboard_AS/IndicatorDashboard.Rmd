...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    #make base map
    p <- ggplot() + 
      geom_rect(aes(xmin = ext$xmin, xmax = ext$xmax, ymin = ext$ymin, ymax = ext$ymax), #rectangle for coloring ocean #change ymin to fit AS domain -20
...
output$map_desc <- renderUI( 
  ...
  if (input$indicator == "CO2") {
    ...
    markdown("This map shows the climatological average SST (1985-2009).  The data in the graphs above is averaged over the Main Hawaiian islands.")
  } else if (input$indicator == "Chl") {
    markdown("This map shows the climatological average ocean color (1998-2009).  The data in the graphs above is averaged over the Main Hawaiian islands.")    
  }
)
