...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    #make base map
    p <- ggplot() + 
      geom_rect(aes(xmin = ext$xmin, xmax = ext$xmax, ymin = -22, ymax = ext$ymax), #rectangle for coloring ocean #change ymin to fit AS domain -25
                color = NA, fill = oceans[1])  +
      geom_rect(aes(xmin = 7.5, xmax = 15, ymin = -17.6, ymax = -10.5), color = "black", fill = NA, linewidth = 0.5) + #AS box
      annotate("text", x = 7, y = -10, label = "American Samoa Grid", size = 3.2, color = "black") +
